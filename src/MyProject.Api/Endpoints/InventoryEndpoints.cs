...
/// <summary>Order management API endpoints — all routes require authentication.</summary>
public static class OrderEndpoints
{
    ...
    public static RouteGroupBuilder MapOrderEndpoints(this WebApplication app)
    {
        ...
        group.MapGet("/my-orders", GetMyOrders)
            ...
            .WithDescription("Returns all orders placed by the current user")
            .Produces<IEnumerable<OrderResponse>>();

        group.MapPost("/", CreateOrder)
            .WithName("CreateOrder")
            .WithSummary("Place a new order")
            .WithDescription("Places a new order for an inventory item")
        ...
        group.MapGet("/", GetAllOrders)
            ...
            .WithDescription("Returns all orders in the system. Admin only.")
            .Produces<IEnumerable<OrderDetailResponse>>();

        group.MapPut("/{id:guid}/status", UpdateOrderStatus)
            .RequireAuthorization(policy => policy.RequireRole("Admin"))
            .WithName("UpdateOrderStatus")
            .WithSummary("Update order status (Admin)")
            .WithDescription("Updates the fulfillment status of an order. Admin only.")
    ...
    {
        var userId = httpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!;
        var orders = await mediator.Send(new GetUserOrdersQuery(userId), ct);
        ...
        CancellationToken ct)
    {
        ...
        var userId = httpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!;
        var result = await mediator.Send(
            new CreateOrderCommand(request.InventoryItemId, request.QuantityRequested, userId, request.Notes), ct);

        return result.IsSuccess
            ? Results.Created($"/api/v1/orders/{result.Value!.Id}", result.Value)
            : Results.Problem(title: "Order creation failed", detail: result.Error, statusCode: StatusCodes.Status400BadRequest);
    }
        ...
        CancellationToken ct)
    {
        ...
        var result = await mediator.Send(
            new UpdateOrderStatusCommand(id, request.Status, request.Notes), ct);

        return result.IsSuccess
            ? Results.Ok(result.Value)
            : Results.Problem(title: "Update failed", detail: result.Error, statusCode: StatusCodes.Status400BadRequest);
    }
}
